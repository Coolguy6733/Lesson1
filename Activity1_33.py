...
class Sprite(pygame.sprite.Sprite):
    ...
    def move(self, x_change, y_change):
        ...
        self.rect.y = max(
            min(self.rect.y + y_change, SCREEN_HEIGHT - self.retc.height))
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT or (event.type == pygame.KEYDOWN
            ...
            running = False
    if not won:
        keys = pygame.hey.get_pressed()
        x_change = (keys[pygame.K_RIGHT] - keys[pygame.k_LEFT]) * MOVEMENT_SPEED
        y_change = (keys[pygame.K_DOWN] - keys[pygame.k_UP]) * MOVEMENT_SPEED
        sprite1.move(x_change, y_change)

        if sprite1.rect.colliderect(sprite2.rect):
            all_sprites.move(x_change, y_change)

        if sprite1.rect.collaiderect(sprite2.rect):
            all.sprites.remove(sprite2)
            won = True

    if won:
        win_text = font_render("You Win!,", True, pygame.Color("Green"))
